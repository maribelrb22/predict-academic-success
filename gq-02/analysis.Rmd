---
title: ¿Cómo influye la situación macroeconómica del país en las carreras elegidas por los alumnos?
De
author: "Jose Luis Alonso y Fernando Alonso"
date: "2023-03-09"
...
#Preliminary view to know the limits of the values
```{r}
...
ui <- fluidPage(
  titlePanel("Variación de las carreras elegidas por los estudiantes en función de la situación macroeconómica del país"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output) {
  ...
  output$career_plot <- renderPlotly({
    ...
    #Filter data, sets sliders their max and min values and transform coded values into courses names
    data_filtered <- data %>%
    ...
    #Transform into characters for showing them in plot
    data_filtered$Course <- as.character(data_filtered$Course) 
    
    #Generate plot
    plot_ly(data_filtered, x = ~Course, y = ~count, type = "bar", text = ~count,
          marker = list(color = ~count, colorscale = "Blues"), 
          textfont = list(size = 14, color = "white"))  %>%
      layout(title = "Carreras elegidas en función de la situación macroeconómica",
